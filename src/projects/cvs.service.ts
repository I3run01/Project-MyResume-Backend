...
export class CvsService {
  ...
  async updateField(cvId: string, field: string, body: any): Promise<CvsDocument> {
    const updateObject = {};
    updateObject[field] = body;
  
    const cv = await this.cvModel.findByIdAndUpdate(cvId, updateObject, { new: true }).exec();
